...
def get_contrib_dates_from_query_res(query_res):
    contrib_dates = []
    contrib_weeks = query_res['data']['user']['contributionsCollection']['contributionCalendar']['weeks']
...
def get_all_contib_dates(username_to_copy_from, start_date, api_key):
    ...
    while start_date <= today:
        ...
        query = QUERY_TEMPLATE % (
            username_to_copy_from, start_date.isoformat(), end_date.isoformat())
...
def main():
    ...
    username_to_copy_from = request_user_input("Enter the GitHub username to copy contributions from: ")

    start_date = request_user_input("Enter the start date for copying contributions (YYYY-MM-DD): ")
    start_date = datetime.date.fromisoformat(start_date)

    api_token = request_user_input("Enter the API token for github (create at https://github.com/settings/tokens): ")

    default_repo_name = 'contrib-copy-{}'.format(username_to_copy_from)
    repo = request_user_input(
        "Enter the name of the repository to use (default: {}): ".format(default_repo_name))
    ...
    print('Create a new(!) repo named {0} at {1} and run the script'.format(repo, GITHUB_BASE_URL))
